...
The project "Spaß mit Flaggen" is a web-based application where users can test their knowledge of national flags. The system presents a random flag, and the user must guess the corresponding country. The application is implemented using a architecture consisting of a frontend, backend, and persistence layer.
